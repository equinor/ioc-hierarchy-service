...
# Only set nuget feed if FEED_ACCESSTOKEN is set
RUN if [ -n "$FEED_ACCESSTOKEN" ]; then \
      wget -q -O /usr/local/bin/nuget.exe https://dist.nuget.org/win-x86-commandline/latest/nuget.exe \
      && mono /usr/local/bin/nuget.exe sources add -name "ioc-cpp-packages" -Source ${FEED_URL} -Username "docker" -Password ${FEED_ACCESSTOKEN} \
      ; fi
# Only set these ENV if FEED_ACCESSTOKEN is set
ENV NUGET_CREDENTIALPROVIDER_SESSIONTOKENCACHE_ENABLED=${FEED_ACCESSTOKEN:+true}
ENV VCPKG_BINARY_SOURCES=${FEED_ACCESSTOKEN:+clear;nuget,ioc-cpp-packages,readwrite}
